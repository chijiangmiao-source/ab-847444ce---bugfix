...
3. **HTTP 冒烟**：对运行中的服务验证
   [菱形旁路] [串联关键点] [平行边] [不可达终端] [96 条独立旁路的宽合流]
   五种拓扑及 422 错误定位。宽合流拓扑（R 经 A 到 M，另有 96 条旁路各自
   经唯一继电器汇入 M，M 后接 97 个终端）专门核对：M 的立即支配者必须是 R
   而非 A，A 与所有旁路继电器均非关键继电器，M 是唯一关键继电器且支配 97
   个终端，所有终端可达且立即支配者均为 M。
